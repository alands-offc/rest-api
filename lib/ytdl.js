 ...
 class YT {
     ...
     static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
         try {
             ...
             const file = await toAudio(stream)
             if (Object.keys(metadata).length !== 0) {
                 await this.WriteTags(file.data, metadata)
             }
             if (autoWriteTags) {
                 await this.WriteTags(file.data, { Title: videoDetails.title, Album: videoDetails.author.name, Year: videoDetails.publishDate.split('-')[0], Image: videoDetails.thumbnails.slice(-1)[0].url })
             }
             return {
                 meta: {
